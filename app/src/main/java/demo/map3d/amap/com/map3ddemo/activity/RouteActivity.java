...
import overlay.ToastUtil;
import overlay.Utils;
import overlay.WalkRouteOverlay;

/**
 * Route路径规划: 驾车规划、骑行规划、步行规划
 */
...
//		getIntentData();
    ...
    private void setfromandtoMarker() {
        aMap.addMarker(new MarkerOptions()
                .position(Utils.convertToLatLng(mStartPoint))
                .icon(BitmapDescriptorFactory.fromResource(R.drawable.start)));
        aMap.addMarker(new MarkerOptions()
                .position(Utils.convertToLatLng(mEndPoint))
    ...
    private void init() {
        ...
        mRide = (ImageView) findViewById(R.id.route_ride);
    ...
    private void drive() {
        ...
        mRide.setImageResource(R.drawable.ride);
        mWalk.setImageResource(R.drawable.route_walk_normal);
        mapView.setVisibility(View.VISIBLE);
    }
    ...
    public void searchRouteResult(int routeType, int mode) {
        ...
        if (routeType == ROUTE_TYPE_RIDE) {// 骑车路径规划
            RideRouteQuery query = new RideRouteQuery(fromAndTo);// 路径规划骑行模式查询
        ...
        if (errorCode == AMapException.CODE_AMAP_SUCCESS) {
            if (result != null && result.getPaths() != null) {
                if (result.getPaths().size() > 0) {
                    ...
                    String des = Utils.getFriendlyTime(dur) + "(" + Utils.getFriendlyLength(dis) + ")";
